...
description: "Participant-held (Nostr) vs edge-held Ed25519 key custody for AI agents: identity vs authority, rotation after compromise, decentralized egress under enterprise controls, three-leg resilience, and a 20-term glossary."
...
**If you are deciding what to actually build first:** [Server-Side Function Tools with AI Runners](https://persephonepunch.github.io/crm-sync-setup/server-side-function-tools.html) is the additive entry point — bounded functions that resolve authority per call and record themselves, with no migration to own. [AI Trust Framework Requirements](https://persephonepunch.github.io/crm-sync-setup/ai-trust-framework-requirements.html) gives the ten requirements and the changes that close them without rewriting an endpoint; [The Trust Framework](https://persephonepunch.github.io/crm-sync-setup/trust-framework.html) is the same argument for a non-technical reader.
...
| Product | The problem it solves for an enterprise | Runs inside |
|---|---|---|
| [CRM Sync App](https://www.crm-sync.dev/products/download-app) | Capability lives behind a URL rather than a binary, so identity, consent, and agent permissions reach the people who need them — legal, compliance, finance — without an endpoint-management project. Your admin key is generated by you and shown once. | Browser / PWA; Cloudflare; Xano |
| [Verified Transaction Layer](https://www.crm-sync.dev/products/verified-transactions) | "Prove this happened, on these terms, at this time" — answered with a signed certificate a third party checks against your public key, instead of a screenshot or a database row you control. | Cloudflare (signing); Xano (record) |
| [Channel Publish](https://www.crm-sync.dev/products/channel-publish) | Attribution stops being rented. Links and QR codes are minted on your own edge, scans count in your ledger, and the order still lands as a normal Shopify order — no third-party link service holding your traffic data. | Shopify; Cloudflare; GA4 → BigQuery |
| [3D Publisher](https://www.crm-sync.dev/products/3d-publisher) | Digital assets ship as revocable per-buyer entitlements rather than files that leak. A rotation kills every copy issued so far without a recall. | Shopify; Cloudflare; R2 |
| [Sample 3D Model](https://www.crm-sync.dev/products/asset-sample-model) | A $19-scale way to put the gated-asset mechanism in front of a security reviewer before committing to anything larger. | Shopify |
| [Firmware Security](https://www.crm-sync.dev/products/firmware-security) | Firmware distribution becomes a grant instead of a shared URL: encrypted vault, ceremony-signed certificates, and a hash-chained record of every grant, denial, and served byte — the evidence chain CRA Article 14 reporting presumes. | Cloudflare; Xano; R2 |
| [SBOM Registry](https://www.crm-sync.dev/products/sbom-registry) | A *current* CycloneDX SBOM per release, generated in CI from the lockfile — so the bill of materials matches the build instead of drifting from it, with a disclosure path for authorities and customers. | Your CI; Cloudflare; Xano |
| [Firmware SBOM — Per-Image Analysis](https://www.crm-sync.dev/products/sbom-firmware-image) | Covers the case regulators actually ask about on connected devices: an SBOM for the firmware image itself, including vendor images with no source, with the image vaulted. | Cloudflare; Xano; R2 |
| [CRA Readiness Assessment](https://www.crm-sync.dev/products/cra-readiness) | Answers "are we the manufacturer, which category are we in, and which ten documents must exist" before the 11 September 2026 reporting date and 11 December 2027 full-obligation date — as a fixed-fee engagement, not an open retainer. | Advisory |

The full catalogue lives at [crm-sync.dev/pages/products](https://www.crm-sync.dev/pages/products), and the knowledge base — including this document — at [crm-sync.dev/pages/knowledge-base](https://www.crm-sync.dev/pages/knowledge-base).

### What a procurement review actually has to clear

- **No new client software.** Nothing to package, sign, or deploy to managed devices; the surface is a URL that existing web policy already governs.
- **No new egress destinations of consequence.** Traffic resolves to vendors already on the approved list, over ordinary HTTPS, inspectable by existing controls.
- **No new data-residency question** beyond ones already answered for the underlying platforms, because the system of record is your own tenancy.
- **No proprietary verification tooling.** Auditors verify with a `GET` and a published key — no licence, no account, no NDA.
- **No lock-in on the evidence.** The record is exportable and the verification key is public, so the proof outlives the commercial relationship.

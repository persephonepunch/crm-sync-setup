...
**Partner Dashboard era:** Apps received non-expiring offline access tokens (`legacy token*`). Store the token once, use forever.

**Dev Dashboard era:** New public apps MUST use expiring tokens:
- Access token (`OAuth access token*`): 60-minute lifetime
...
|-------|---------------|-----------------|----------------|
| Token type | `legacy token*` (never expires) | `OAuth access token*` (60 min) + refresh token (90 day) | `refreshShopifyTokenIfNeeded()` implemented |
...
- [ ] **Implement expiring tokens** — replace `legacy token*` with `OAuth access token*` + refresh token flow

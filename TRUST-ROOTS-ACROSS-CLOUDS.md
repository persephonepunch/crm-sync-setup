...
**Webflow** is moving from CMS collections bound to a template toward a Data API with components and variables — a queryable site graph, but still render-first, so the decision stays outside it. **Xano** is moving from REST endpoints, one per shape, toward GraphQL and function-stack management, where the function is the unit — and that is where a real seam appears, because policy finally has somewhere to live. **Cloudflare** moved from being a cache and a perimeter to Workers, D1 and Vectorize, where the function *is* the product and the seam is the whole offering. **Google** is moving from GA4 reports and warehouse tables toward BigQuery and Vertex, and GQL over Spanner Graph — the query shape modernises while governance lags it.

...
- **The upload certificate becomes a signed license** — an attestation of who held what authority, under which terms, from when to when — [verifiable by anyone against the published public key set](https://crm-sync.dev/verify), offline, in the relying party's own browser.
...
The enterprise answer is asymmetric keys with named generations, and it comes down to three steps:

1. **One identity, many credentials.** You are a subject on the platform, not a keyholder. Each credential you hold can be revoked on its own — nobody else is disturbed.
2. **Rotation without a flag day.** Signing keys have named generations: old and new public keys are published side by side, so yesterday's records keep verifying while new ones use the new key.
3. **Teams as roles, not shared secrets.** A team is a set of scoped roles with separation of duties — and every grant is [verifiable by anyone against the published key set](https://crm-sync.dev/verify), in their own browser.

Two models, two audiences: relay for the individual with a fleet of agents; entitlement for the team with duties to separate. Both ride the same tide — authority becoming something signed, scoped, and verifiable rather than merely possessed. SBOM registries and CRA evidence chains live on the enterprise side of that line.

...
Don't take this article's word for any of it: paste a mandate or entitlement token into the offline verifier at [crm-sync.dev/verify](https://crm-sync.dev/verify). The check runs in your own browser against the public key — the platform's API is never contacted, and if a cap, scope, or subject had been altered after signing, the signature would fail on your machine.
...
![Against Liquid-served JS — six properties that differ between theme JavaScript and compiled Functions: trust boundary, logic disclosure, data exposure, execution guarantee, change control, consent surface.](https://crm-sync.dev/kb/media/docs/against-liquid-served-js.png)
*Why the WASM wrapper matters — six properties where in-page JavaScript and platform-executed Functions differ: trust boundary, logic disclosure, data exposure, execution guarantee, change control, consent surface.*

Why Shopify? Why Google? Because they need each other. Shopify holds the commerce substrate — catalog, checkout, and the app system that executes trusted code. Google holds the demand substrate — search, segments, and the identity rails buyers already carry. The **Universal Commerce Protocol** for agentic commerce is built on exactly this partnership: an agent discovers through one side, transacts through the other, and does both under a mandate the buyer signed. Building on the pair is not a vendor bet; it is the protocol's own shape.

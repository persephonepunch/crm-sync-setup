...
(function () {
  ...
  /* ---------- styles (scoped .dsx-*) ------------------------------------ */
  var css = '' +
  '.dsx-modal-open{overflow:hidden}' +
  /* modal */
  '.dsx-ov{position:fixed;inset:0;z-index:2147483000;display:none;background:rgba(10,10,10,.55);' +
  'backdrop-filter:saturate(1.4) blur(3px);padding:28px}' +
  '.dsx-ov.dsx-show{display:flex;align-items:center;justify-content:center}' +
  '.dsx-panel{background:#fff;color:#0a0a0a;width:min(1120px,96vw);height:min(90vh,900px);' +
  'display:flex;flex-direction:column;border:1.5px solid #0a0a0a;box-shadow:0 30px 80px rgba(0,0,0,.35)}' +
  '.dsx-bar{display:flex;align-items:center;gap:6px;padding:10px 12px;border-bottom:1.5px solid #0a0a0a;' +
  'flex-wrap:wrap}' +
  '.dsx-eyebrow{font:600 11px/1 "Roboto Mono",ui-monospace,Menlo,monospace;letter-spacing:.22em;' +
  'text-transform:uppercase;color:#5c5c5c;margin-right:8px;padding-left:4px}' +
  '.dsx-tab{font:600 11px/1 "Roboto Mono",ui-monospace,Menlo,monospace;letter-spacing:.1em;' +
  ...
  '.dsx-tab:hover{color:#0a0a0a;border-color:#0a0a0a}' +
  '.dsx-tab.dsx-on{color:#fff;background:#0a0a0a;border-color:#0a0a0a}' +
  '.dsx-bar .dsx-sp{flex:1 1 auto}' +
  '.dsx-x{display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;' +
  'border:1px solid #0a0a0a;background:#fff;color:#0a0a0a;cursor:pointer;border-radius:2px}' +
  '.dsx-x:hover{background:#0a0a0a;color:#fff}' +
  '.dsx-frame{flex:1 1 auto;width:100%;border:0;background:#fff}' +
  /* nav items injected inline */
  '.dsx-burger{display:none;align-items:center;justify-content:center;width:42px;height:38px;' +
  'border:1.5px solid #0a0a0a;background:#fff;color:#0a0a0a;cursor:pointer;border-radius:2px;margin-left:8px}' +
  '.dsx-burger:hover{background:#0a0a0a;color:#fff}' +
  /* mobile drawer */
  '.dsx-drawer{position:fixed;top:0;right:0;bottom:0;z-index:2147482000;width:min(320px,86vw);' +
  ...
  '.dsx-drawer.dsx-show{transform:none}' +
  '.dsx-drawer .dsx-dhead{display:flex;align-items:center;justify-content:space-between;' +
  'padding-bottom:14px;margin-bottom:10px;border-bottom:1.5px solid #0a0a0a}' +
  '.dsx-drawer .dsx-dh{font:600 11px/1 "Roboto Mono",ui-monospace,Menlo,monospace;letter-spacing:.22em;' +
  'text-transform:uppercase;color:#5c5c5c}' +
  '.dsx-drawer a,.dsx-drawer button.dsx-link{display:block;text-align:left;width:100%;' +
  ...
  '.dsx-drawer a:hover,.dsx-drawer button.dsx-link:hover{color:#5c5c5c}' +
  '.dsx-drawer .dsx-dcta{margin-top:16px;border:0;background:#0a0a0a;color:#fff;text-align:center;' +
  'font:600 14px/1 "Roboto",sans-serif;padding:15px;border-radius:2px;text-decoration:none;display:block}' +
  '.dsx-drawer .dsx-dcta:hover{opacity:.85}' +
  '.dsx-scrim{position:fixed;inset:0;z-index:2147481000;background:rgba(10,10,10,.4);display:none}' +
  '.dsx-scrim.dsx-show{display:block}' +
  '@media (max-width:' + BP + 'px){' +
  '  ' + NAV_SEL + '{display:none!important}' +
  '  ' + CTA_SEL + '{display:none!important}' +
  '  .dsx-burger{display:inline-flex}' +
  '  .dsx-navitem-inline{display:none!important}' +
  '  .dsx-ov{padding:0}' +
  '  .dsx-panel{width:100vw;height:100vh;border:0}' +
  '}';
  ...
  function buildModal() {
    ...
    ov = document.createElement('div'); ov.className = 'dsx-ov'; ov.setAttribute('role', 'dialog');
    ov.setAttribute('aria-modal', 'true'); ov.setAttribute('aria-label', 'CRM Sync documentation');
    var panel = document.createElement('div'); panel.className = 'dsx-panel';
    var bar = document.createElement('div'); bar.className = 'dsx-bar';
    bar.innerHTML = '<span class="dsx-eyebrow">Docs</span>';
    tabsWrap = document.createElement('span'); tabsWrap.style.cssText = 'display:flex;gap:6px;flex-wrap:wrap';
    TABS.forEach(function (t, i) {
      var b = document.createElement('button'); b.className = 'dsx-tab'; b.textContent = t.label;
      b.addEventListener('click', function () { select(i); });
      tabsWrap.appendChild(b);
    });
    bar.appendChild(tabsWrap);
    var sp = document.createElement('span'); sp.className = 'dsx-sp'; bar.appendChild(sp);
    var x = document.createElement('button'); x.className = 'dsx-x'; x.setAttribute('aria-label', 'Close');
    x.innerHTML = ICON.close; x.addEventListener('click', close); bar.appendChild(x);
    frame = document.createElement('iframe'); frame.className = 'dsx-frame';
  ...
  function select(i) {
    var tabs = tabsWrap.querySelectorAll('.dsx-tab');
    tabs.forEach(function (t, k) { t.classList.toggle('dsx-on', k === i); });
  ...
  function open(which) {
    ...
    select(i);
    ov.classList.add('dsx-show'); document.documentElement.classList.add('dsx-modal-open');
  }
  function close() {
    if (ov) ov.classList.remove('dsx-show');
    document.documentElement.classList.remove('dsx-modal-open');
  }
  ...
  ready(function () {
    ...
    if (nav) {
      ...
      docs.className = (sibClass + ' dsx-navitem-inline').trim();
      ...
      login.className = (sibClass + ' dsx-navitem-inline').trim();
    ...
    burger.className = 'dsx-burger'; burger.setAttribute('aria-label', 'Menu');
    ...
    var scrim = document.createElement('div'); scrim.className = 'dsx-scrim';
    var drawer = document.createElement('div'); drawer.className = 'dsx-drawer';
    var head = document.createElement('div'); head.className = 'dsx-dhead';
    head.innerHTML = '<span class="dsx-dh">Menu</span>';
    var dx = document.createElement('button'); dx.className = 'dsx-x'; dx.setAttribute('aria-label', 'Close menu');
    dx.innerHTML = ICON.close; head.appendChild(dx); drawer.appendChild(head);

    // clone the existing nav links
    if (nav) {
      nav.querySelectorAll('a').forEach(function (a) {
        if (a.classList.contains('dsx-navitem-inline')) return; // skip our injected inline ones
    ...
    // Docs (opens modal)
    var dDocs = document.createElement('button'); dDocs.className = 'dsx-link'; dDocs.textContent = 'Docs';
    ...
    // CTA (mirrored)
    if (cta) {
      var dCta = document.createElement('a'); dCta.className = 'dsx-dcta';
      dCta.href = cta.getAttribute('href') || '#'; dCta.textContent = cta.textContent.trim();
      drawer.appendChild(dCta);
    }
    document.body.appendChild(scrim); document.body.appendChild(drawer);

    function openDrawer() { drawer.classList.add('dsx-show'); scrim.classList.add('dsx-show');
      burger.setAttribute('aria-expanded', 'true'); }
    function closeDrawer() { drawer.classList.remove('dsx-show'); scrim.classList.remove('dsx-show');

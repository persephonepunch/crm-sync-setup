...
<h3 class="uk-heading-small uk-text-secondary">The server that decides never gets built</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Revlon, in documented order</h3>
...
<h3 class="uk-heading-small uk-text-secondary">The two rungs operators misjudge</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Perimeter IT answers one question: who got in</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Server-scoping is controllership, not networking</h3>
...
<h3 class="uk-heading-small uk-text-secondary">The remedy converts drift into catastrophe</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Platform programs never land consent on a server</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Chronological-lapse integrators vs. the real-time record</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Webhooks vs. the socketed ID — the API-level distinction</h3>

Name the two shapes at the API level and the category choice stops being abstract. A **webhook** is an after-the-fact callback: the platform POSTs a JSON *copy* of an event to a URL, and an ESB — Boomi, MuleSoft, the WebSphere lineage — subscribes, queues, transforms, and forwards it on the bus's schedule. The consent state rides along as a **flag detached from its moment**; by the time the copy lands in the CRM the subject may have revoked, and the interval between is exactly the rung-2 mechanism. A **socketed ID** is the opposite shape: Shopify's [Customer Privacy API](https://shopify.dev/docs/api/customer-privacy) binds consent to the visitor's live session at the moment it is given — `setTrackingConsent` writes it, `currentVisitorConsent` answers from it — and every event that leaves the storefront carries the consent state and the session identity *together*. Wired directly into BigQuery, that stream is the shape AI can be trusted with: **session-keyed, consent-stamped, vectorized and revenue-weighted at the row level** — data whose permission travels with it, instead of a nightly batch of flags about data that already moved. [Webhooks](https://shopify.dev/docs/api/webhooks) still have a job — they are how destinations hear that something happened. The failure is making the *bus* the system of record for a *moment*.

<h3 class="uk-heading-small uk-text-secondary">AI inherits the shackles</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Why the right-size fix is dismissed</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Evidence you issue about yourself is testimony</h3>
...
<h3 class="uk-heading-small uk-text-secondary">2026: the doors held, the doors' customers fell</h3>
...
<h3 class="uk-heading-small uk-text-secondary">A copy of authority outliving the record of it</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Containment by surface area: keys, never the bridge</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Bus-factor-one is a controls problem</h3>
...
<h3 class="uk-heading-small uk-text-secondary">One build graph is the maximum surface</h3>
...
<h3 class="uk-heading-small uk-text-secondary">Distributed islands with component publishing</h3>

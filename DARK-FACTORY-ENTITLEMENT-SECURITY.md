...
*Diagram companion: [BIM Fortress Exposure vs the Event-Socket Heal](https://crm-sync.dev/share/bim-fortress-event-socket) — the fortress wound map and the heal, in two charts. Delivery-side companion: [Data Layer — WordPress, Astro, Headless](https://www.crm-sync.dev/pages/knowledge-base#wordpress-web-components) covers the same architecture from the storefront direction. Setup: [CRM Sync Setup Reference](https://www.crm-sync.dev/pages/knowledge-base#setup-guide).*
